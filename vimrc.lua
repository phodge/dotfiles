...
local function expand(expr)
  return vim.api.nvim_call_function('expand', {expr})
end

local function exists(what)
  return vim.api.nvim_call_function('exists', {what})
end

local function has(feature)
  return vim.api.nvim_call_function('has', {feature})
end
...
local function executable(name)
  return vim.api.nvim_call_function('executable', {name})
end

local function filereadable(name)
  return vim.api.nvim_call_function('filereadable', {name})
end

local function Plug(what)
  return vim.api.nvim_command(string.format("Plug '%s'", what))
end


if not exists('g:vim_peter') then
  let('vim_peter', 1)
end

if not exists('g:hackymappings') then
  let('hackymappings', 0)
end
...
if exists('&termguicolors') then
  ...
  if not has('nvim') then
    -- init.lua can only be exuected by neovim anyway
...
elseif has('nvim') and eval('&term') == 'screen' then
  -- TODO: what was this for?
  setglobal('term', 'screen-256color')
end

-- we need to work out what tmux session we're running under
let('tmux_session', '')
if executable('tmux') and exists('*systemlist') then
  -- NOTE: I experienced a bug where vim 8.0.007's systemlist() would return an
  -- empty list, but only during startup
  local result = vim.api.nvim_call_function('systemlist', {'tmux display-message -p "#S"'})
  if #result then
    let('tmux_session', result[1])
  end
end

-- runtimepath modifications {{{

  if eval('g:allow_rtp_modify') then

    -- if we are using neovim, we have to manually add ~/.vim to rtp
    if has('nvim') then
      setglobal('runtimepath', '~/.vim,' .. eval('&runtimepath') .. ',~/.vim/after')
    end

    -- add our own vim/ and vim-multi-python/ folders to runtimepath
    local specials = {'vim', 'vim-multi-python', 'vim-git-magic'}
    local name
    for i, name in ipairs(specials) do
      local local_ = expand('<sfile>:h')..'/'..name
      setglobal('runtimepath',
          string.format('%s,%s,%s/after', local_, eval('&runtimepath'), local_))
    end

    -- add the <vim-est> thing if it is present
    if eval("isdirectory(expand('~/src/vim-est.git'))") then
      setglobal('runtimepath', eval('&runtimepath') .. ',' .. expand('~/src/vim-est.git'))
    end

    -- vimstatic area for things we may download manually
    setglobal('runtimepath', '~/.vimstatic,' .. eval('&runtimepath'))
  end
...
local plugpath = eval('$HOME') .. '/.vim/autoload/plug.vim'

if filereadable(plugpath) then
  -- use different path depending on vim/nvim
  local plugins = '~/.vim/plugged'
  if has('nvim') then
    local plugins = '~/.nvim/plugged'
  end

  vim.api.nvim_call_function('plugmaster#begin', {'~/src/plugedit', '~/src', plugins}) -- {{{

  if get('tmux_session') == 'SPACETEA-DEV' then
  ...
  let('jedi#use_splits_not_buffers', "winwidth")
  ...
  if has('nvim') and eval("get(g:, 'peter_use_ycm', 0)") then
  ...
  if haS('signs') then
    ...
    vim.api.nvim_command('nnoremap <space>g :GitGutterToggle<CR>')
    ...
    setglobal('updatetime', 250)
  else
    vim.api.nvim_command('nnoremap', "<space>g :echoerr 'vim-gitgutter is not installed'<CR>")
  end
  ...
  vim.api.nvim_command("PlugMaster 'phodge/vim-javascript-syntax'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-python-syntax'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-jsx'")
  vim.api.nvim_command("PlugMaster 'phodge/MicroRefactor'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-shell-command'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-syn-info'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-split-search'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-auto-spell'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-vimui'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-myschema'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-vcs'")
  vim.api.nvim_command("PlugMaster 'phodge/vim-hiword', {'on': []}")
